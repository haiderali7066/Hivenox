...
const accentStyles: Record<
  Accent,
  { text: string; chipBg: string; panelBg: string; panelBorder: string; glow: string; solidBg: string; railActiveBg: string }
> = {
  blue: {
    text: 'text-blue-600',
...
const productsItems: NavItem[] = [
  { title: 'Results as a Service', href: '/solutions/results-as-a-service', icon: Rocket, desc: 'Guaranteed outcomes powered by our technology — you pay for results, not seats.' },
...
const allProductsItems: NavItem[] = [
  ...
  { title: 'Hivenox AI Sales Agent', href: '/products/ai-sales-agent', icon: Bot, desc: 'Prospects, follows up, and books meetings automatically.' },
  { title: 'Hivenox AI Support Agent', href: '/products/ai-support-agent', icon: Headset, desc: 'Answers customers instantly, in Arabic and English.' },
  { title: 'Hivenox AI Recruiter', href: '/products/ai-recruiter', icon: Briefcase, desc: 'Screens candidates and schedules interviews for you.' },
  { title: 'Hivenox Voice AI', href: '/products/voice-ai', icon: Mic, desc: 'Makes and takes natural phone calls, around the clock.' },
  { title: 'Hivenox Insights AI', href: '/products/insights-ai', icon: BarChart3, desc: 'Turns raw business data into plain-language answers.' },
]
...
const menuOverviews = {
  products: { href: '/solutions', label: 'All Solutions', prefix: '/solutions', accent: 'blue' as Accent, eyebrow: 'Products' },
  aiWorkers: { href: '/ai-workers', label: 'All AI Workers', prefix: '/ai-workers', accent: 'violet' as Accent, eyebrow: 'AI Workers' },
  allProducts: { href: '/products', label: 'All Apps', prefix: '/products', accent: 'indigo' as Accent, eyebrow: 'All Products' },
  erpApps: { href: '/erp-solutions', label: 'All ERP Apps', prefix: '/erp-solutions', accent: 'emerald' as Accent, eyebrow: 'ERP Apps' },
  industries: { href: '/industries', label: 'All Industries', prefix: '/industries', accent: 'amber' as Accent, eyebrow: 'Industries' },
  company: { href: '/about', label: 'About Hivenox', prefix: '/about', accent: 'rose' as Accent, eyebrow: 'Company' },
} as const
...
export function SiteHeader() {
  ...
  return (
    <header
      ...
      className={cn(
        ...
        scrolled || openMenu ? 'border-slate-200/60 bg-white/95 shadow-sm backdrop-blur-lg' : 'border-transparent bg-white/70 backdrop-blur-md',
        mobileOpen && 'border-transparent bg-white'
      )}
    >
      {/* SLIM UTILITY BAR */}
      <div className={cn('hidden overflow-hidden border-b border-slate-100 transition-all duration-300 ease-out xl:block', scrolled || openMenu ? 'max-h-0 opacity-0' : 'max-h-9 opacity-100')}>
        <div className="mx-auto flex h-8 max-w-7xl items-center justify-between px-6 lg:px-8">
          <p className="text-xs font-medium text-slate-500">🇦🇺 Australia · 🇦🇪 UAE · 🇸🇦 Saudi Arabia — one platform, every region</p>
  ...
  >
    ...
    <MenuTrigger
      ...
      onToggle={() => setOpenMenu((m) => (m === 'allProducts' ? null : 'allProducts'))}
    />
    <MenuTrigger
      label="ERP Apps"
      ...
      onToggle={() => setOpenMenu((m) => (m === 'erpApps' ? null : 'erpApps'))}
    />
    <MenuTrigger
      label="Industries"
    ...
    {mobileOpen ? <X className="size-5" aria-hidden="true" /> : <Menu className="size-5" aria-hidden="true" />}
      ...
      <div
        className={cn(
          ...
          activeAccent === 'emerald' && 'from-transparent via-emerald-500 to-transparent',
      ...
      >
        <div className="relative min-h-full p-8 sm:p-10 lg:p-12 xl:p-14 xl:pl-20 2xl:pl-28">
          <div className={cn('pointer-events-none absolute -top-16 right-0 size-72 rounded-full blur-3xl', accentStyles[activeAccent].glow)} aria-hidden="true" />

          {openMenu === 'products' && <PanelHeading eyebrow={menuOverviews.products.eyebrow} accent="blue" />}
          {openMenu === 'products' && <RailMenu items={productsItems} accent="blue" overviewHref={menuOverviews.products.href} overviewLabel={menuOverviews.products.label} />}

          {openMenu === 'aiWorkers' && <PanelHeading eyebrow={menuOverviews.aiWorkers.eyebrow} accent="violet" />}
          {openMenu === 'aiWorkers' && <RailMenu items={aiWorkersItems} accent="violet" overviewHref={menuOverviews.aiWorkers.href} overviewLabel={menuOverviews.aiWorkers.label} />}

          {openMenu === 'allProducts' && <PanelHeading eyebrow={menuOverviews.allProducts.eyebrow} accent="indigo" />}
          {openMenu === 'allProducts' && <RailMenu items={allProductsItems} accent="indigo" overviewHref={menuOverviews.allProducts.href} overviewLabel={menuOverviews.allProducts.label} />}

          {openMenu === 'erpApps' && <PanelHeading eyebrow={menuOverviews.erpApps.eyebrow} accent="emerald" />}
          {openMenu === 'erpApps' && <RailMenu items={erpAppsItems} accent="emerald" overviewHref={menuOverviews.erpApps.href} overviewLabel={menuOverviews.erpApps.label} />}

          {openMenu === 'industries' && <PanelHeading eyebrow={menuOverviews.industries.eyebrow} accent="amber" />}
          {openMenu === 'industries' && <RailMenu items={industriesItems} accent="amber" overviewHref={menuOverviews.industries.href} overviewLabel={menuOverviews.industries.label} />}

          {openMenu === 'company' && <PanelHeading eyebrow={menuOverviews.company.eyebrow} accent="rose" />}
      ...
      >
        <div className="flex min-h-full flex-col px-5 py-6">
          ...
          <div className="flex-1 space-y-1">
            <Link href="/" onClick={() => setMobileOpen(false)} className="block border-b border-slate-100 px-3 py-3.5 text-base font-bold text-slate-900">
              Home
            </Link>
            <MobileAccordion title="Products" overviewHref={menuOverviews.products.href} items={productsItems} accent="blue" />
            <MobileAccordion title="AI Workers" overviewHref={menuOverviews.aiWorkers.href} items={aiWorkersItems} accent="violet" />
            <MobileAccordion title="All Products" overviewHref={menuOverviews.allProducts.href} items={allProductsItems} accent="indigo" />
            <MobileAccordion title="ERP Apps" overviewHref={menuOverviews.erpApps.href} items={erpAppsItems} accent="emerald" />
            ...
            <MobileAccordion title="Company" overviewHref={menuOverviews.company.href} items={companyItems} accent="rose" />
          </div>

          <div className="mt-8 space-y-3 border-t border-slate-100 pt-6">
            <Link href="/book-demo" onClick={() => setMobileOpen(false)} className="flex w-full items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-3.5 text-base font-bold text-slate-700">
              Book Demo
            </Link>
            <Link
              href="/signup"
...
function PanelHeading({ eyebrow, accent }: { eyebrow: string; accent: Accent }) {
  return (
    <div className="mb-8 flex items-center gap-3">
      <span className={cn('h-2.5 w-2.5 rounded-full', accentStyles[accent].chipBg)} aria-hidden="true" />
      <h2 className="text-3xl font-black tracking-tight text-slate-900">{eyebrow}</h2>
    </div>
  )
}
...
function RailMenu({
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1.2fr_1fr] lg:gap-12">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? styles.railActiveBg : 'text-slate-600 hover:bg-slate-50',
                focusRing
              )}
            >
              <span className="flex items-center gap-2 truncate">
                <span className="truncate transition-transform group-hover:translate-x-0.5">{item.title}</span>
                {item.badge && <span className={cn('shrink-0 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide', styles.chipBg)}>{item.badge}</span>}
              </span>
              <ChevronRight className={cn('size-4 shrink-0 text-slate-300 transition-transform duration-200 group-hover:translate-x-1', isActive && styles.text)} aria-hidden="true" />
            </Link>
          )
        })}
        <Link
      ...
      <div className={cn('relative flex flex-col justify-center overflow-hidden rounded-3xl border p-8', styles.panelBg, styles.panelBorder)}>
        <div className={cn('absolute inset-x-0 top-0 h-1.5', styles.solidBg)} aria-hidden="true" />
        <span className={cn('mb-4 flex size-12 items-center justify-center rounded-xl', styles.chipBg)}>
        ...
        <h3 className="mb-1.5 flex items-center gap-2 text-lg font-bold text-slate-900">
          {current.title}
          {current.badge && <span className={cn('rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide', styles.chipBg)}>{current.badge}</span>}
        </h3>
        <p className="mb-6 text-sm leading-relaxed text-slate-600">{current.desc}</p>
        <Link href={current.href} className={cn('inline-flex w-fit items-center gap-1.5 rounded-full px-5 py-2.5 text-sm font-bold text-white shadow-md transition-colors', styles.solidBg)}>
          Explore {current.title} <ArrowRight className="size-3.5" aria-hidden="true" />
        </Link>
      </div>
    </div>
  )
    ...
    <div className="grid gap-8 lg:grid-cols-2 lg:gap-10">
      <div className="grid gap-3 sm:grid-cols-2">
        {companyItems.map((item) => (
          ...
          >
            <span className={cn('flex size-10 items-center justify-center rounded-xl', styles.chipBg)}>
      ...
      <div className={cn('relative flex flex-col justify-between overflow-hidden rounded-3xl border p-8', styles.panelBg, styles.panelBorder)}>
        <div className={cn('absolute inset-x-0 top-0 h-1.5', styles.solidBg)} aria-hidden="true" />
        <div className="relative">
          <h3 className="text-xl font-black text-slate-900">Book a Demo</h3>
          <p className="mt-2 text-sm leading-relaxed text-slate-600">See how Hivenox works for your team. Get a personalised walkthrough with one of our solution architects.</p>
        </div>
        <Link
          href="/book-demo"
          className={cn(
  ...
  return (
    <div className="border-b border-slate-100">
      <div className="flex items-center justify-between">
        {overviewHref ? (
          <Link href={overviewHref} onClick={() => setOpen(false)} className="flex-1 py-3.5 pl-3 text-base font-bold text-slate-900">
            {title}
          </Link>
        ) : (
        ...
        <button
          ...
          aria-expanded={open}
          className={cn('flex items-center justify-center rounded-full px-3 py-3.5', focusRing)}
        >
      ...
      <div className={cn('grid overflow-hidden transition-all duration-300 ease-in-out', open ? 'grid-rows-[1fr] pb-4 opacity-100' : 'grid-rows-[0fr] opacity-0')}>
        <div className="flex flex-col gap-1 overflow-hidden px-3">
          {overviewHref && (
            <Link href={overviewHref} className={cn('mb-1 inline-flex items-center gap-1.5 rounded-xl px-2 py-2 text-sm font-bold', styles.railActiveBg)}>
          ...
          {items.map((item) => (
            ...
            >
              <span className="flex items-center gap-3 truncate">
                <span className={cn('flex size-7 shrink-0 items-center justify-center rounded-lg', styles.chipBg)}>
